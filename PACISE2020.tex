...
} \title{{CoNFET:} AN ENGLISH SENTENCE TO EMOJIS TRANSLATION ALGORITHM} \author{Alex Day$^1$ \and
  Chris Mankos $^{1}$
  \and
  Soo Kim$^1$
  \and
...
A score for the overall n-gram sequence is generated by adding up the cosine similarity of each n-gram composing the sequence and then dividing by the number of n-grams as shown in Equation \refeq{eq:averageCosineDifference}.
...
\begin{enumerate}[label= (\arabic*)]
  ...
  \end{enumerate}
  \item
  For each pair of the machine translated sentence and the user translated sentence:
  \begin{enumerate}[topsep=0pt]
    \itemsep0em
    \item
